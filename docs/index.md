...
The Tide project is an [official part of the WordPress project](https://make.wordpress.org/tide/) and still in the early stages. Today, Tide scans all WordPress plugins and themes and provides scan results as JSON through the Tide API.

The next step is to start using the JSON output in the WordPress plugin repository to show PHP compatibility information. This will help drive the adoption of newest PHP versions as both plugin authors and WordPress site owners installing plugins will be more aware of the PHP compatibility of each plugin.
...
* The WPTide.org website witch includes both the documentation you are reading and also servers the JSON API endpoints from where the reports can be queried. Visit https://www.wptide.org/search to see examples of JSON reports.

In addition we are working on the meta repository of WordPress.org to handle fetching the reports generated by Tide and showing summaries of it in the WordPress.org plugin and theme directories.
...
- Go packages used by the server: https://github.com/wptide/pkg
